...
run(function()
	...
	local anims = {
		...
		["CustomGX+"] = {
			{CFrame = CFrame.new(0.39, 1, 0.2) * CFrame.Angles(math.rad(-30), math.rad(50), math.rad(-90)), Time = 0.20},
			{CFrame = CFrame.new(0.39, 1, 0.2) * CFrame.Angles(math.rad(-30), math.rad(50), math.rad(-90)), Time = 0.04},
			{CFrame = CFrame.new(0.7, 0.1, 0.59) * CFrame.Angles(math.rad(-84), math.rad(50), math.rad(-38)), Time = 0.06},
			{CFrame = CFrame.new(0.7, 0.1, 0.59) * CFrame.Angles(math.rad(-84), math.rad(50), math.rad(-38)), Time = 0.010},
			{CFrame = CFrame.new(0.39, 0.1, 1.37) * CFrame.Angles(math.rad(-84), math.rad(50), math.rad(-38)), Time = 0.20}
		},
		Exhibition = {
	...
	local function sendHit(target)
		local args = {
			[1] = target,
			[2] = "hit"
		}
		bedwars.Server:FireServer(unpack(args))
	end
...
--[[run(function()
    local AntiHit = {Active = false, Interval = 4}
    local cloneRoot, originalRoot
    local bounceCooldown = tick()
    local eventConnections = {}

    local function spawnClone()
        repeat task.wait() until isAlive(lplr, true) and store.matchState ~= 0 or not AntiHit.Active
        if not AntiHit.Active then return end

        lplr.Character.Parent = game
        originalRoot = lplr.Character:WaitForChild("HumanoidRootPart")
        cloneRoot = originalRoot:Clone()
        cloneRoot.Parent = lplr.Character
        lplr.Character.PrimaryPart = cloneRoot
        originalRoot.Parent = workspace
        originalRoot.Transparency = 1
        entityLibrary.character.HumanoidRootPart = cloneRoot
    end

    local function removeClone()
        lplr.Character.Parent = game
        originalRoot.Parent = lplr.Character
        cloneRoot.Parent = nil
        lplr.Character.PrimaryPart = originalRoot
        cloneRoot:Destroy()
        cloneRoot, originalRoot = nil, nil
    end

    AntiHit = GuiLibrary.ObjectsThatCanBeSaved.BlatantWindow.Api.CreateOptionsButton({
        Name = "AntiHit",
        HoverText = "Makes it more challenging for enemies to land hits.",
        Function = function(isActivated)
            AntiHit.Active = isActivated
            if isActivated then
                spawnClone()

                table.insert(eventConnections, lplr.CharacterAdded:Connect(spawnClone))
                table.insert(eventConnections, runservice.RenderStepped:Connect(function()
                    if isAlive(lplr, true) and lplr.Character.PrimaryPart == cloneRoot and tick() >= bounceCooldown then
                        originalRoot.Velocity = Vector3.zero
                        originalRoot.CFrame = cloneRoot.CFrame
                    end
                end))

                spawn(function()
                    while AntiHit.Active do
                        if killauraNearPlayer and tick() > bounceCooldown and lplr.Character.PrimaryPart == cloneRoot then
                            bounceCooldown = tick() + 0.4
                            for _ = 1, 5 do
                                if not killauraNearPlayer then
                                    bounceCooldown = tick()
                                    break
                                end
                                originalRoot.CFrame += Vector3.new(0, 1000, 0)
                                local startInterval = tick() + (0.01 * AntiHit.Interval)
                                repeat task.wait() until tick() >= startInterval or not AntiHit.Active
                                originalRoot.Velocity = Vector3.new(cloneRoot.Velocity.X, -1, cloneRoot.Velocity.Z)
                                originalRoot.CFrame = cloneRoot.CFrame
                            end
                        end
                        task.wait()
