...
local vapeAssetTable = {["vape/assets/VapeCape.png"] = "rbxassetid://126010322157642", ["vape/assets/ArrowIndicator.png"] = "rbxassetid://13350766521"}
...
run(function()
	...
	function whitelist:playeradded(v, joined)
		if self:get(v) ~= 0 then
			...
			if self.localprio == 0 then
				...
				GuiLibrary.SelfDestruct = function() warningNotification('GX+', 'Cant Self Destruct because a Private or a whitelisted member is in your match', 10) end
	...
	function whitelist:checkmessage(msg, plr)
		...
		if self.localprio > 0 and self.said[plr.Name] == nil and msg == 'helloimusinginhaler' and plr ~= lplr then
			...
			notif('GX+', plr.Name..' is using vape!', 60)
	...
	function whitelist:check(first)
		local whitelistloaded, err = pcall(function()
			local _, subbed = pcall(function() return game:HttpGet('https://github.com/YoGalaxyStar13/Whitelists') end)
			local commit = subbed:find('spoofed_commit_check')
			commit = commit and subbed:sub(commit + 21, commit + 60) or nil
			commit = commit and #commit == 40 and commit or 'main'
			whitelist.textdata = game:HttpGet('https://raw.githubusercontent.com/YoGalaxyStar13/Whitelists/'..commit..'/PlayerWhitelist.json', true)
		end)
