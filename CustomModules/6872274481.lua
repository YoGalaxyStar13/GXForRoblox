...
local LowHopEnabled = false
local LowHopHeight = 16
local CFrameSpeed = 21
...
local function enableLowHop()
    ...
    RunService.RenderStepped:Connect(function()
        if not LowHopEnabled then return end

        local moveDirection = character.Humanoid.MoveDirection
        local newVelocity = Vector3.new(moveDirection.X * CFrameSpeed, math.sin(tick() * 5) * LowHopHeight, moveDirection.Z * CFrameSpeed)
...
local lowHopButton = GuiLibrary.ObjectsThatCanBeSaved.BlatantWindow.Api.CreateOptionsButton({
    Name = 'LowHop',
    HoverText = 'Originally made by Tencity 5.0 (Discontinued) devs this is just a remake!',
    Function = function(calling)
        if calling then
            LowHopEnabled = true
            enableLowHop()
            warningNotification("LowHop Warning", "Don't use this module to fly around as this will flag when you're not on the ground.", 5)
        else
            LowHopEnabled = false
            disableLowHop()
        end
